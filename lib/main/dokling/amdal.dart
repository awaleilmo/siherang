...
class amdalPage extends StatefulWidget {
  ...
  amdalPage({required this.tit, required this.ur});
...
class _amdalPage extends State<amdalPage> {
  ...
  Future<void> kaon() async {
    var url = Uri.https(linknya.urlbase, "app/dokling?nama=AMDAL");
  ...
  Future<Null> notifikasi() async {
    ...
    String userId = prefs.getString('id') ?? '';
    var url = Uri.https(
        linknya.urlbase, "app/clearnotif?userId=" + userId + "&menu=1");
    await http.get(url);
  }
